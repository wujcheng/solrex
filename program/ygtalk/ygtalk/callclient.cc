...
"  who          Prints the online friends from your roster.\n"
"  chat <name>  Text chat with name (somebody@gmail.com).\n"
"  send <name>  Send file to name (somebody@gmail.com).\n"
...
//  console_->Print(line);
  ...
  if ((words.size() == 1) && (words[0] == "roster")) {
    ...
    return;
  }else if((words.size() == 1) && (words[0] == "who")){
    console_->Print("who");
    console_->Send("who\n");
    PrintWho();
    return;
    ...
    MakeCallTo(words[1]);
    return;
  } else if ((words.size() >= 3) && (words[0].find("chat") != std::string::npos)             && (words[1].find("@gmail.com") != std::string::npos)){
    console_->Print("chat");
    std::string texte;
...
CallClient::CallClient(buzz::XmppClient* xmpp_client)
    : xmpp_client_(xmpp_client), roster_(new RosterMap),
      file_roster_(new RosterMap), enligne_roster_(new RosterMap),
      all_roster_(new RosterMap), call_(NULL), incoming_call_(false),
      incoming_file_(false), sending_file_(false){
...
void CallClient::OnFileReceived(const std::string& from, 
       const std::string& file, const std::string& no_use)
{
   std::string str;
   str += from;
