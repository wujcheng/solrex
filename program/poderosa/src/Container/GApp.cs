...
namespace Poderosa
{
	...
	internal class GApp
	{
		...
		private static IntPtr _globalMutex; //複数起動時の設定ファイル保護
		...
		private static void Init(InitialAction act, string[] args, bool already_exists) { //GFrameの作成はコンストラクタの後にしないと、GuevaraAppのメソッドをデリゲートの引数にできない。
