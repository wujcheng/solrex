/*
 ...
 $Id: ProfileEdit.cs,v 1.2 2005/04/20 09:06:04 okajima Exp $
*/
using System;
	...
	internal class ProfileEdit : System.Windows.Forms.Form
	{
		...
		private void OnOK(object sender, EventArgs args) {
			...
			try {
				...
				if(_sshHostBox.Text.Length==0) throw new Exception(Env.Strings.GetString("Message.ProfileEdit.EmptySSHServer"));
				_result.SSHHost = _sshHostBox.Text;
				if(_accountBox.Text.Length==0) throw new Exception(Env.Strings.GetString("Message.ProfileEdit.EmptyAccount"));
				...
				if(_result.AuthType==AuthenticationType.PublicKey) _result.PrivateKeyFile = _privateKeyBox.Text;
				...
				if(_localToRemoteOption.Checked) {
					...
					if(_LRRemoteHostBox.Text.Length==0) throw new Exception(Env.Strings.GetString("Message.ProfileEdit.EmptyRemoveHost"));
