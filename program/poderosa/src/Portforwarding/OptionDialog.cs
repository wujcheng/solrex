...
* $Id: OptionDialog.cs,v 1.2 2005/04/20 09:08:27 okajima Exp $
*/
using System;
...
namespace Poderosa.PortForwarding
{
	...
	internal class OptionDialog : System.Windows.Forms.Form
	{
		...
		/// コード エディタで変更しないでください。
		/// this._styleBox.Items.AddRange(EnumDescAttribute.For(typeof(GFrameStyle)).DescriptionCollection());
		/// this._badCharBox.Items.AddRange(EnumDescAttribute.For(typeof(BadCharOption)).DescriptionCollection());
		/// </summary>
		private void InitializeComponent()
		{
			...
			this._sshPage.BackColor = ThemeUtil.TabPaneBackColor;
		...
		private bool CommitConnectionOptions() {
			...
			try {
				...
				foreach(string c in _socksNANetworksBox.Text.Split(';')) {
					if(!Communication.NetUtil.IsNetworkAddress(c)) throw new FormatException();
				}
		...
		private bool CommitGenericOptions() {
			...
			try {
				...
				if(Env.Options.OptionPreservePlace!=(OptionPreservePlace)_optionPreservePlace.SelectedIndex && !Env.IsRegistryWritable) {
					Util.Warning(this, Env.Strings.GetString("Message.OptionDialog.RegistryAuthRequired"));
					return false;
				}
				_options.OptionPreservePlace = (OptionPreservePlace)_optionPreservePlace.SelectedIndex;

				_options.Language = (Language)_languageBox.SelectedIndex;
