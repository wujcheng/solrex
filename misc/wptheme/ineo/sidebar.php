...
) ) : ;echo '		<div class="widget widget_ads clearfix">
			';if($options['showcase_caption']) : ;echo '				<div class="title">';if($options['showcase_title']){echo($options['showcase_title']);}else{_e('Showcase','inove');};echo '</div>
			';endif;;echo '			<ul>
			';
...
;echo '			</ul>
	...
	';endif;;echo '
	<!-- Posts View  -->
	';if (is_home() &&function_exists('get_most_viewed')): ;echo '		<div class="widget">
			<div class="title">Hot Posts</div>
		   <ul>
			  ';get_most_viewed('post',5);;echo '		   </ul>
		</div>
	';elseif (is_tag() &&function_exists('get_most_viewed_tag') &&!empty($tag_id)): ;echo '		<div class="widget">
			<div class="title">Hot Posts</div>
			<ul>
			  ';get_most_viewed_tag($tag_id,'post',5);;echo '			</ul>
		</div>
	';elseif (is_category() &&function_exists('get_most_viewed_category') &&!empty($cat)): ;echo '		<div class="widget">
			<div class="title">Hot Posts</div>
			<ul>
			';get_most_viewed_category($cat,'post',5);;echo '			</ul>
		</div>
	';elseif (function_exists('get_most_viewed')) : ;echo '		<div class="widget">
			<div class="title">Hot Posts</div>
		   <ul>
			  ';get_most_viewed('post',5);;echo '		   </ul>
		</div>
	';endif;;echo '
	<!-- recent comments -->
	';if( function_exists('wp_recentcomments') ) : ;echo '		<div class="widget">
			<div class="title">Recent Comments</div>
			<ul>
				';wp_recentcomments('limit=5&length=16&post=false&smilies=true');;echo '			</ul>
		</div>
	';endif;;echo '
	<!-- tag cloud -->
	<div id="tag_cloud" class="widget">
		<div class="title">Tag Cloud</div>
		';wp_tag_cloud('smallest=8&largest=16');;echo '	</div>

';endif;;echo '<!-- sidebar north END -->
...
<div class="widget clearfix">
	...
	';if ( !function_exists('dynamic_sidebar') ||!dynamic_sidebar('east_sidebar') ) : ;echo '
		...
		<div class="widget_categories">
			...
			<ul>
				';wp_list_cats('sort_column=name&optioncount=0&depth=1');;echo '			</ul>
		</div>
	...
	';if ( !function_exists('dynamic_sidebar') ||!dynamic_sidebar('west_sidebar') ) : ;echo '
		...
		';if(function_exists('wp_easyarchives_widget')) : ;echo '			';wp_easyarchives_widget("mode=none&limit=6");;echo '		';else : ;echo '			<ul>
				';wp_get_archives('type=monthly');;echo '			</ul>
		';endif;;echo '
	';endif;;echo '	</div>
	<!-- sidebar west END -->
...
';if ( !function_exists('dynamic_sidebar') ||!dynamic_sidebar('south_sidebar') ) : ;echo '
	...
	<div class="widget">
		...
		<ul>
			';wp_register();;echo '			<li>';wp_loginout();;echo '</li>
		</ul>
	</div>

';endif;;echo '<!-- sidebar south END -->
