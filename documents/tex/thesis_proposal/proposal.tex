...
\item 寻找更稳健的安全定位算法。在异常节点的识别和排除方面，目前比较多的是使用统计和选举的手段，我们希望能寻找更合适的算法，使定位算法所需计算量更小而不影响定位精度。

\item 设计开销与隐私保护更均衡的位置私密性保护方案。目前的位置信息私密性保护方案主要采用在网络中使用虚假路由或者引入虚假流量来迷惑敌手，这样所带来的通信开销比较大，我们希望能找到一个更均衡的私密性保护方案。

\item 结合考虑安全定位算法与其它安全方案。由于安全定位算法与其它的安全方案可以共享一些信息和模块，我们可以将它们结合一起考虑，也许会发现更优的算法。
