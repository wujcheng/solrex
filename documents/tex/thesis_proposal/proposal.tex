...
由于受以上条件限制，很多传统的安全措施难以使用在无线传感器网络上，而且无线传感器网络往往用于监测敏感信息或者部署在充满敌意的环境中，所以无线传感器网络在应用中面临着许多不同的安全问题和安全需求，如图~\ref{wsn_sec_map}~所示。

\begin{enumerate}

\item 数据安全。在无线传感器网络中，数据安全包括数据的保密性、完整性、新鲜性和数据源认证。数据的保密性要求节点获得的信息不泄露给其它节点、敏感信息传输安全以及抵抗流量分析；数据的完整性要求在传输的过程中数据不被攻击者篡改；数据的新鲜性要求保证收到的数据是最新的而不是攻击者重放的；数据源认证则使消息的接收者能够验证数据来源于其声称的发送者。数据安全一般通过密码学手段来达到~\cite{Perrig2002}，同时这些密码学方法也是保证其它安全需求的基础。

\item 密钥管理和访问控制。由于计算和能量要求的限制，经典的密钥交换算法或公钥算法无法使用在无线传感器网络中，那么就造成密钥分配的困难。有效和安全的密钥管理，是达到网络可用性、鲁棒性和自组织的重要因素~\cite{Lee2007}；传感器节点能量受限，合理地采用睡眠、侦听等访问控制方法能够有效地延长无线传感器网络的运行寿命。安全的时钟同步在访问控制中也起着非常重要的作用。

\item 路由安全。无线传感器网络一般组成树形结构，数据的传输通过多跳转发，如何避免敌手对路由协议的攻击，保证路由的安全、正确，是影响网络可用性的重要因素。
...
\subsubsection{无线传感器网络中的位置相关安全服务}  

在无线传感器网络监控的应用中，监控程序获得网络所收集到的环境信息，检测是否有事件发生。在一些事件发生时，传感器网络往往需要知道该事件发生的位置以及感知该事件的传感器节点的位置。因此，位置信息是无线传感器网络中的一项重要信息，有着非常广泛的应用，包括数据的识别和关联、节点定位、对指定区域节点的管理和查询、评估节点密度和覆盖范围、网络能量分布图、基于地理位置的路由、目标追踪和其它位置应用~\cite{Boukerche2007a}。这里我们主要讨论位置信息的安全获得及其在其它安全领域的应用。

位置验证(Location Verificaiton)是安全定位研究中最重要的一个
